...
/**
 * Pharmacy Staff — Verify Pre-Authorization / Prescription Code
 */
...
import { ClipboardCheck, Search, Loader2, CheckCircle2, AlertCircle, Pill } from "lucide-react";
...
export default function PharmacyVerifyPage() {
  ...
  return (
    <HospitalSidebarWrapper currentPath="/hospital/pharmacy/verify" role={UserRole.PHARMACY}>
      <div className="min-h-screen bg-background overflow-auto p-6 max-w-3xl mx-auto space-y-6">
        <div className="border-b pb-4">
          <h1 className="text-2xl font-semibold flex items-center gap-2">
            <ClipboardCheck className="h-6 w-6 text-primary" /> Verify Prescription Authorization
          </h1>
          <p className="text-sm text-muted-foreground">Validate HMO authorization codes before dispensing medications</p>
        </div>

        <Card className="data-visualization">
          <CardHeader>
            <CardTitle className="text-base">Enter Authorization Code</CardTitle>
            <CardDescription>Enter the code provided by the patient or doctor</CardDescription>
          </CardHeader>
          <CardContent>
            ...
            {result && (
              <Alert className="mt-4 bg-green-500/10 border-green-500/30">
                ...
                <AlertDescription className="mt-2 space-y-2 text-sm">
                  ...
                  <p><strong>Approved Medication/Service:</strong> {result.serviceName || "Prescription Dispense"}</p>
                  <p><strong>HMO Provider:</strong> {result.hmoName || "ClearMed HMO"}</p>
                  <Badge className="bg-green-600 text-white mt-2">Ready for Dispense</Badge>
                </AlertDescription>
              </Alert>
            )}
          </CardContent>
