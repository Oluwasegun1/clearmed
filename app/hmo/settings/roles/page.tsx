...
/**
 * HMO Admin — Roles & Permissions matrix.
 * /hmo/settings/roles
 */
...
import { HMO_PERMISSIONS, type HmoPermissionKey } from "@/lib/permissions";
import {
...
interface OrgRole {
  id: string;
  name: string;
  permissions: HmoPermissionKey[];
  isDefault: boolean;
}

const ALL_PERMISSIONS = Object.entries(HMO_PERMISSIONS) as [HmoPermissionKey, { label: string }][];

export default function HMORolesPage() {
  ...
  const [dirtyRoles, setDirtyRoles] = useState<Record<string, HmoPermissionKey[]>>({});
  ...
  const togglePerm = (roleId: string, perm: HmoPermissionKey, current: HmoPermissionKey[]) => {
  ...
  return (
    <HMOSidebarWrapper currentPath="/hmo/settings/roles" role={UserRole.HMO_ADMIN}>
      <div className="min-h-screen bg-background overflow-auto">
        ...
        <div className="relative border-b border-border/50 bg-card/30 backdrop-blur-xl px-6 py-4">
          ...
          <p className="text-sm text-muted-foreground">
            Define what each role can do within your HMO. Changes apply immediately on save.
          </p>
        </div>
          ...
          <Card className="data-visualization">
            <CardContent className="p-4 flex items-center gap-3">
              <Input value={newRoleName} onChange={(e) => setNewRoleName(e.target.value)}
                placeholder="New role name (e.g. Senior Auditor)" className="max-w-xs" />
